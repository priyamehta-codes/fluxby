...
export function usePWAInstall(): PWAInstallInfo {
  ...
  // Note: The landing page doesn't have its own manifest, so we check if
  // the browser would generally support PWA installation based on platform/browser
  const supportsPWA = browserSupportsPWA(browser, platform);
